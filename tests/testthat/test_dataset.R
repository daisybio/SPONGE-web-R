...
test_that("Check if dataset information from API are parsed correct", {
  expect_equal(datasetInformation("kidney clear cell carcinoma"),
  ...
  expect_error(datasetInformation("foo"), "API response is empty.*")
  })

test_that("Check if run information from API are parsed correct", {
  expect_equal(runInformation("kidney clear cell carcinoma"),
  ...
  expect_error(runInformation("foo"), "API response is empty.*")
})
